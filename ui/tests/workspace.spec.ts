...
test('searches loaded runs and preserves the comparison selection', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Compare metrics' })).toHaveAttribute(
    'href',
    '/compare?runs=run-001,run-002',
  );
});
...
test('shows actionable authorization errors without inventing data', async ({ page }) => {
  await page.route('**/v1/**', (route) =>
    route.fulfill({
      status: 401,
      json: { error: { message: 'Owner credentials required', request_id: 'request-123' } },
    }),
  );
  await page.goto('/');
